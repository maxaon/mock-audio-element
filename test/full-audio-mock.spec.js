...
import {createFullAudioMock} from '../src/full-audio-mock';
// Environment
process.env.MOCK_AUDIO_ELEMENT_TEST = true;

// Specs
describe('Full audio mock', function () {
  this.timeout(10000);
  let Audio;
  let testkit;
  const FIXTURE_URL = 'test src';
  const AUDIO_DURATION = 120;

  beforeEach(() => {
    const aT = createFullAudioMock();
    Audio = aT.Audio;
    testkit = aT.testkit;
    testkit.setDuration(AUDIO_DURATION);
  });
